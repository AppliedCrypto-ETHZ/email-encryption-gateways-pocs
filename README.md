...
This repository contains proof-of-concept artifacts for _[Why Johnny Should Not Delegate Email Encryption to Gateways](https://eprint.iacr.org/2026/1911)_.
...
| `header-decryption-oracle.py` | Header decryption-oracle PoC. | Install `uv` and the OpenSSL cli. Adjust the hard-coded SMTP host, email addresses, and certificate & private key path if needed, and set up `authbind` to allow receiving the generated DSNs. Then run `./header-decryption-oracle.py`. |
...
| `bounce-padding-oracle.py` | CBC padding-oracle PoC. | Install `uv` and the OpenSSL cli. Adjust the hard-coded SMTP host, email addresses, and certificate path if needed and set up `authbind` to allow receiving the generated DSNs. Then run `./bounce-padding-oracle.py`. |
| `header-decryption-oracle.py` | Header decryption-oracle PoC. | Install `uv` and the OpenSSL cli. Adjust the hard-coded SMTP host, email addresses, and certificate path if needed and set up `authbind` to allow receiving the generated DSNs. Then run `./header-decryption-oracle.py`. |
| `full-message-bounce.py` | Full-message-bounce PoC. | Install the OpenSSL cli, adjust the hard-coded SMTP host, email addresses, and certificate path if needed, then run `./full-message-bounce.py`. |


## Proton Mail Bridge

Both scripts are [mitmproxy](https://mitmproxy.org/) addons that rewrite Bridge's HTTPS API responses as if a malicious Proton server were splicing attacker HTML around a genuine ciphertext. Bridge must trust mitmproxy's CA and send its HTTPS through the proxy:

1. Install deps: `pip install -r requirements.txt`.
2. Start mitmproxy with the chosen addon, e.g. `mitmdump -s boundary-injection.py` (listens on `:8080` by default). The first run writes a CA under `~/.mitmproxy/`.
...
| `efail-inline-disposition-bypass.py` | mitmproxy addon that implements the direct-exfiltration EFAIL variant. | Run with `mitmdump -s efail-inline-disposition-bypass.py` after the setup above. |
...
| File | What it is | How to run/use |
|------|------------|----------------|
| `smime_harness_lib.py` | Shared CMS/ASN.1 helpers, DSN listeners, and config. | Set BOUNCE_RCPT, HARNESS_MAILER_FROM, and PROFILES to match your test setup |
| `padding_oracle_bounce.py` | CBC padding-oracle via DSN bounce PoC, including gateway-specific optimizations. | Install `uv`, the OpenSSL cli, and set up `authbind` to allow receiving the generated DSNs. Configure smime_harness_lib.py, then run `./padding_oracle_bounce.py <profile> [--specialized]`. |
| `padding_oracle_gcm_downgrade.py` | GCM-to-CBC downgrade padding oracle PoC. | Install `uv` and set up `authbind` to allow receiving the generated DSNs. Configure smime_harness_lib.py, then run `./padding_oracle_gcm_downgrade.py <profile>`. |
| `header_decrypt_gcm_downgrade.py` | GCM-to-CFB downgrade header-decryption oracle PoC. | Install `uv` and set up `authbind` to allow receiving the generated DSNs. Configure smime_harness_lib.py, then run `./header_decrypt_gcm_downgrade.py <profile>`. |
| `certs/*.pem` | Directory for the certificates used by the above scripts | Place the certificates for the targeted email addresses at certs/{seppmail,cisco,ciphermail}.pem, respectively. |
